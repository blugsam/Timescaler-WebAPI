...
public class CsvStructureValidator : AbstractValidator<RawCsvRow>
{
    ...
    public CsvStructureValidator()
    {
        ...
        RuleFor(x => x.Fields[0])
            .NotEmpty()
            .WithMessage("Поле 'Date' не может быть пустым.");

        RuleFor(x => x.Fields[1])
            .NotEmpty()
            .WithMessage("Поле 'ExecutionTime' не может быть пустым.");

        RuleFor(x => x.Fields[2])
            .NotEmpty()
            .WithMessage("Поле 'Value' не может быть пустым.");
    }
}
